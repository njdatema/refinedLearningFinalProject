...
def get_args_parser():
    ...
    parser.add_argument('--data_root', default='./p2pnet_dataset',
                        help='path where the dataset is')
    
    parser.add_argument('--output_dir', default='./log',
                        help='path where to save, empty for no saving')
    parser.add_argument('--checkpoints_dir', default='./ckpt_finetune',
    ...
    parser.add_argument('--pretrained', default='', help='path to pretrained checkpoint')
    parser.add_argument('--freeze_backbone', action='store_true')
    parser.add_argument('--freeze_fpn', action='store_true')
    parser.add_argument('--train_heads_only', action='store_true')
...
def main(args):
    ...
    if args.train_heads_only:
        print("Training heads only: regression + classification")
        for name, param in model_without_ddp.named_parameters():
            param.requires_grad = False

        for name, param in model_without_ddp.regression.named_parameters():
            param.requires_grad = True

        for name, param in model_without_ddp.classification.named_parameters():
            param.requires_grad = True

    elif args.freeze_backbone or args.freeze_fpn:
        ...
        if args.freeze_fpn:
            print("Freezing FPN")
            for name, param in model_without_ddp.fpn.named_parameters():
                param.requires_grad = False
